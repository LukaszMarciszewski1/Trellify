...
const Materials: React.FC<UsedProductsProps> = ({ cardId, boardId, usedProducts }) => {
  const { data: productsApi } = useGetAllProductsQuery()
  const [updateBoard] = useUpdateBoardMutation();
  const [updateCard] = useUpdateCardMutation()
  const [updateProduct] = useUpdateProductMutation()
  const [selectProduct, setSelectProduct] = useState<any>({})
  const [productsList, setProductsList] = useState<ProductModel[]>(usedProducts)
  ...
  const handleAddProductToList = () => {
    ...
    const newProducts = [...productsList, { ...selectProduct, used: 0 }]
  ...
  return (
    <div className={styles.usedProducts}>
      <form className={styles.usedProductsForm} onSubmit={handleSubmitForm}>
        ...
        <div className={styles.productsList}>
          {
            productsList?.map((item) => (
              <div key={item._id} className={styles.formGroup}>
